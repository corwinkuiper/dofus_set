...
function ItemBox({ item, weights, pinned, togglePinned, ban, searching, toggleSearchSlot }: { item: Item, weights: WeightsState, pinned: boolean, togglePinned: () => void, ban: () => void, searching: boolean, toggleSearchSlot: () => void }) {
                ...
                <div className="itembox-data">
                    ...
                    <div className="itembox-bottom-section">
                        ...
                        <div className="itembox-actions">
                            <button className={classNames({ 'itembox-search': true, 'itembox-search-active': searching })} onClick={toggleSearchSlot} />
...
export function BestItemDisplay({ items, weights, setBonuses, pinnedSlots, togglePinned, banItem, searchingSlot, toggleSearchSlot }: { items: (Item | null)[], weights: WeightsState, setBonuses: SetBonus[], pinnedSlots: number[], togglePinned: (slot: number) => void, banItem: (item: Item) => void, searchingSlot: number | undefined, toggleSearchSlot: (slot: number) => void }) {
    return (
        <div className="best-item-display">
            {items.map((item, i) => item &&
                <ItemBox
                    ...
                    key={i}
                    weights={weights}
                    pinned={pinnedSlots.includes(i)}
                    togglePinned={togglePinned.bind(null, i)}
                    ban={banItem.bind(null, item)}
                    searching={searchingSlot === i}
                    toggleSearchSlot={toggleSearchSlot.bind(null, i)} />
            )}
