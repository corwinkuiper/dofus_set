...
import type { WorkerQuery } from "./optimiser-worker";
import type {
  DofusOptimiserSettings,
  DofusOptimiserConfig,
  DofusOptimiserResult,
  DofusItem,
  DofusSpellClass,
} from "./types";

export type OptimisationRequest = DofusOptimiserConfig & DofusOptimiserSettings;
...
export class Optimiser {
  ...
  private createWorker() {
    ...
    const worker = new Worker(new URL("./optimiser-worker", import.meta.url), {
      type: "module",
    });
  ...
  async optimise(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.queueJob(
        ...
        (data: unknown) => resolve(data as DofusOptimiserResult),
        reject,
  ...
  async get_items_in_slot(slot: number): Promise<DofusItem[]> {
    return new Promise((resolve, reject) => {
      const jobId = generateId();
      this.queueJob(
        {
        ...
        (data) => resolve(data as DofusItem[]),
        reject,
  ...
  async get_spells(): Promise<DofusSpellClass[]> {
    return new Promise((resolve, reject) => {
      const jobId = generateId();
      this.queueJob(
        {
        ...
        (data) => resolve(data as DofusSpellClass[]),
        reject,
        new AbortController().signal
      );
    });
  ...
  async get_all_items(): Promise<DofusItem[]> {
    return new Promise((resolve, reject) => {
      const jobId = generateId();
      this.queueJob(
        {
...
export const optimiser = new Optimiser();
