...
import { Optimiser } from "./dofus/Optimiser";
...
function OptimisationSettings({
  ...
  updateWeightsState: (newWeightsState: WeightsState) => void;
  maxLevel: number;
  ...
  return (
    <div className="optimisation-settings">
      <div className="optimisation-header">
        <a
          href="https://blog.kuiper.dev/optimisation-of-dofus-equipment-sets"
          target="_blank"
      ...
      <WeightsSelector
        weights={weights}
        updateWeightsState={updateWeightsState}
      />
      <ExoSelector
        exoOptions={weights.exoOptions}
        updateExoOptions={(newOptions: ExoOptions) =>
          updateWeightsState(weights.alterExoOptions(newOptions))
        }
      />
    </div>
  );
...
class App extends React.Component<{}, AppState> {
  ...
  constructor(props: {}) {
    ...
    this.searchApi = new SearchApi(optimiser);
    this.api = new OptimiseApi(optimiser);
  ...
  private async runOptimiser() {
    ...
    try {
      ...
      for (let i = 0; i < 51; i++) {
        const weightValue =
          this.state.weightsState.weights.find((weight) => weight.statId === i)
            ?.weightValue ?? 0;
      ...
      const bestItems = setResult.items.map(
        (item) =>
          item &&
          new Item(
            item.name,
      ...
      const setBonuses = setResult.setBonuses.map(
        (bonus) =>
          new SetBonus(bonus.name, bonus.characteristics, bonus.numberOfItems)
      );
      this.setState({
        bestItems,
        setBonuses,
        resultingCharacteristics: setResult.overallCharacteristics,
      });
    } finally {
  ...
  unbanItem(item: Item) {
    this.setState({
      bannedItems: this.state.bannedItems.filter(
        (i) => i.dofusId !== item.dofusId
      ),
    });
  }
      ...
      <div className="app-container">
        <div className="weights-container">
          <OptimisationSettings
            ...
            updateWeightsState={this.updateWeightsState}
            maxLevel={this.state.maxLevel}
            setMaxLevel={this.setMaxLevel}
          />
          <button
            className="optimise-button"
            disabled={this.state.optimising || this.state.rateLimited}
            onClick={this.runOptimiser}
          >
            Optimise!
        ...
        <OverallCharacteristics
          characteristics={this.state.resultingCharacteristics}
        />
      </div>
    );
  }
